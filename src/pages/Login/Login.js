...
const Login = () => {
    ...
    const { isLoading, authError, emailLogin, signInWithGoogle } = useAuth();
    ...
    const handleLoginSubmit = e => {
        emailLogin(loginData.email, loginData.password, location, navigate);
        e.preventDefault();
    }
    ...
    return (
        <Box
            sx={{
                height: '100vh',
                background: `linear-gradient(90deg, #FFFFFF 50%, ${partialBgColor} 50%)`,
                display: 'flex',
                justifyContent: 'center',
                alignItems: 'center',
                position: 'relative'
